...
def getContracts(client: AlgodClient) -> Tuple[bytes, bytes]:
    ...
    if len(APPROVAL_PROGRAM) == 0:
        APPROVAL_PROGRAM = __fullyCompileContract(client, approval_program())
        CLEAR_STATE_PROGRAM = __fullyCompileContract(client, clear_state_program())
...
def __fullyCompileContract(client: AlgodClient, contract: Expr) -> bytes:
